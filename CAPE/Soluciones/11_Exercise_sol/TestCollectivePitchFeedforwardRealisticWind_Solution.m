...
Count    	= c(:,1);
Range     	= c(:,2);
DEL_MyT_FB  = (sum(Range.^WoehlerExponent.*Count)/N_REF).^(1/WoehlerExponent);
                                            
%% Brute-Force-Optimization
T_Buffer_v  = 2.8;%[2:0.2:4];                                            	% needs adjustment !!!
...
for i_T_Buffer=1:n_T_Buffer
    ...
    Count                           = c(:,1);
    Range                           = c(:,2);
    DEL_MyT_FF(i_T_Buffer)          = (sum(Range.^WoehlerExponent.*Count)/N_REF).^(1/WoehlerExponent);
end
