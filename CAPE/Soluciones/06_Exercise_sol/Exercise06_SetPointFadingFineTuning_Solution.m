% -----------------------------
% Script: Performs a fine tuning of the set-point-fading.
...
Parameter                       = NREL5MWDefaultParameter_FBSWE_Ex6_SPF_Solution(Parameter);
...
for iDelta_P = 1:nDelta_P
    ...
    Parameter.SPF.Delta_P       = Delta_P_v(iDelta_P);
    ...
    for iURef = 1:nURef
        ...
        sim('NREL5MW_FBSWE_SLOW2DOF_Ex6_SPF_Solution.mdl')
    ...
    AEP(iDelta_P)  = sum(Weights.*P_mean)*8760;                                     % Please adjust !!!
    DEL(iDelta_P)  = sum(Weights.*DEL_MyT.^WoehlerExponent).^(1/WoehlerExponent);   % Please adjust !!!
...
ylabel('AEP [GWh]','interpreter','latex')
xlabel('$\Delta P$ [MW]','interpreter','latex')
subplot(212)
box on; hold on
plot(Delta_P_v/1e6,DEL/1e6,'.-','Markersize',20)
plot([min(Delta_P_v) max(Delta_P_v)]/1e6,[1 1]*43,'k')
ylabel('DEL($M_{y,\textrm{T}}$) [MNm]','interpreter','latex')
xlabel('$\Delta P$ [MW]','interpreter','latex')
legend('DLC 1.2','load margin')
