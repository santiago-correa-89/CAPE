...
S_LL        = 1/4*( S_uu.*(2+2*gamma_uu) + S_vv*(y_n_1^2/x_n_1^2 + y_n_2^2/x_n_2^2 ));
...
% collect gamma first point
SUM_gamma_1u       	= zeros(size(f));       % allocation
for iPoint=1:1:nPoint                       % ... all iPoints
    if IsInRotorDisc(iPoint)
        Distance        = ((y_1-Y(iPoint))^2+(z_1-Z(iPoint))^2)^0.5;
        SUM_gamma_1u    = SUM_gamma_1u + exp(-kappa.*Distance);
    end
end

% collect gamma second point
SUM_gamma_2u       	= zeros(size(f));       % allocation
for iPoint=1:1:nPoint                       % ... all iPoints
    if IsInRotorDisc(iPoint)
        Distance        = ((y_2-Y(iPoint))^2+(z_2-Z(iPoint))^2)^0.5;
        SUM_gamma_2u    = SUM_gamma_2u + exp(-kappa.*Distance);
    end
end

% cross-spectra rotor-effective wind speed and its lidar estimate
S_RL                = 1/nPointInRotorDisc/2*S_uu.*(SUM_gamma_1u+SUM_gamma_2u);

%% 9. Coherence
gamma_Sq_RL         = abs(S_RL).^2./S_RR./S_LL;
k                   = 2*pi*f/URef;
MCB                 = interp1(gamma_Sq_RL,k,0.5);
SDES                = 2*pi/MCB/(2*R); 
